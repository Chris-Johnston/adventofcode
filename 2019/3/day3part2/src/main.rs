    ...
    for p1 in &wire1.0
    {
        for p2 in &wire2
        {
            if p1.0 == p2.0 && p1.1 == p2.1
            {
                ...
                {
                    println!("found new min {:?} {:?} = {}", p1, p2, steps);
                    min_steps = steps;
                }
            }
        }
...
fn get_set(input_str: &str) -> (Vec<(isize, isize, isize)>, collections::HashSet<(isize, isize)>)
{
    // let w1_instructions = "R8,U5,L5,D3";
    let w1_instructions = input_str.split(",");

    let mut cur = (0, 0, 0); // x, y, steps
    let mut set = Vec::new();
    // set.insert((0, 0));
    ...
    for instruction in w1_instructions
    {
        ...
        for _ in 0..distance
        {
            ...
            println!("Adding {:?}", cur);
            set.push(cur);
            points.insert((cur.0, cur.1));
        }
    }
    return (set, points);
}

fn join_set(input_str: &str, wire1_points: &collections::HashSet<(isize, isize)>) -> Vec<(isize, isize, isize)>
{
    // let w1_instructions = "R8,U5,L5,D3";
    let w1_instructions = input_str.split(",");
    ...
    for instruction in w1_instructions
    {
        ...
        for _ in 0..distance
        {
            ...
            if wire1_points.contains(&(cur.0, cur.1))
            {
                println!("merging {:?}", cur);
                set.push(cur);
            }
