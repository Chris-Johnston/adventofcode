...
def get_potentials(shortest_paths, interested_paths, remaining_time, visited_set):
    ...
    # print(len(potentials), 'potentials returned?')
    # return potentials
    return potentials[3:]
...
def get_path_tree_start():
    global max_score, score_max_vals
    ...
    current_path.path_set = set()
    ele_path = Path([], 'AA')
    ele_path.path_set = set()

    ele_path.time_left = 26
    current_path.time_left = 26

    end_paths = []
    already_considered = set()

    get_path_tree_recurse(current_path, ele_path, end_paths, interested_paths, already_considered)

    # 2405 is too low
    # not 2408
    # not 2246
    print("answer", max_score, score_max_vals)
    assert max_score != 2524
    assert max_score != 2425

    # print(current_path)
    # print('end paths', end_paths)
    max_path = 0
    max_path_v = None
    for me, ele in end_paths:
        combined_score = me.score + ele.score
        print("end path", combined_score, "path", me.path, ele.path)
        if combined_score > max_path:
            max_path = combined_score
            max_path_v = (me, ele)
...
def get_path_tree_recurse(current: Path, elephant: Path, end_paths: list, interested_paths, already_considered):
    global max_score
    global iter
    global top_scores
    global prediction_iters
    global score_max_vals
    iter += 1

    if iter % 5000 == 0:
    # print("looking at path", current.score, current.time_left, current.path)
        print(iter, '/', prediction_iters, '(', iter/ prediction_iters, ')', max_score, 'checking. scores:', current.score + elephant.score, (current.score, elephant.score), 'time:', (current.time_left, elephant.time_left), 'paths:', current.path, elephant.path)

    combined_score = current.score + elephant.score
    if combined_score > max_score:
        assert current.time_left >= 0 and elephant.time_left >= 0
        max_score = combined_score
        score_max_vals = (current, elephant)

    if abs(len(current.path) - len(elephant.path)) > 2:
        return None

    # if len(current.path) > 3 and current.score < top_scores[0]:
    #     return None
    # else:
    #     top_scores.append(current.score)
    
    # if len(elephant.path) > 3 and elephant.score < top_scores[0]:
    #     return None
    # else:
    #     top_scores.append(elephant.score)
    
    # top_scores.sort()
    # top_scores = top_scores[:100]

    closed = current.path_set.union(elephant.path_set)

    if len(closed) == len(interested_paths):
        # print('stop')
        return None

    interested_paths = interested_paths.difference(closed)
    # print("interested in ", interested_paths, "keys")

    # p = find_paths(current.current_location)
    p = get_shortest_path(current.current_location)
    current_potentials = get_potentials(p, interested_paths, current.time_left, closed)

    assert len(current_potentials) < 60

    # child_branch_insert = 0

    current_child_branches = []

    for next, vent, time_cost in current_potentials:
        if vent <= 0:
            # no point
            continue
        # handle current
        ...
        if next in closed:
            # already closed
            continue
        if next in elephant.path_set:
            continue
        child_path = Path(current.path.copy(), next)
        child_path.path.append(next)
        child_path.path_set = set(child_path.path)
        ...
        # child_branch_insert += 1
        # current.child_branches.insert(0, child_path)
        current_child_branches.append(child_path)

    # print("Inserted child branch", len(current_child_branches))

    prediction_iters += len(current_child_branches)
    for child in current_child_branches:
        get_path_tree_recurse(child, elephant, end_paths, interested_paths, already_considered)


    ele_child_branches = []

    p = get_shortest_path(elephant.current_location)
    # assume potentials are sorted by time cost
    elephant_potentials = get_potentials(p, interested_paths, elephant.time_left, closed)
    
    # print(elephant_potentials)

    assert len(elephant_potentials) < 60

    for next, vent, time_cost in elephant_potentials:
        if vent <= 0:
            # no point
            continue
        # handle current
        if (time_cost + 1) > elephant.time_left:
            # not enough time for this
            # print("moving to", next, "uses more time", time_cost, "than avail", current.time_left)
            continue
        if next in closed:
            # already closed
            continue
        if next in current.path_set:
            continue
        child_path = Path(elephant.path.copy(), next)
        child_path.path.append(next)
        child_path.path_set = set(child_path.path)
        child_path.score = elephant.score + vent
        # include time to open valve
        child_path.time_left = elephant.time_left - time_cost - 1
        child_path.current_location = next

        # elephant.child_branches.insert(0, child_path)
        ele_child_branches.append(child_path)
    
    prediction_iters += len(ele_child_branches)
    for ele in ele_child_branches:
        get_path_tree_recurse(current, ele, end_paths, interested_paths, already_considered)
        
        # # handle elephant
        # if (ele[2] + 1) > elephant.time_left:
        #     # not enough time for this
        #     # print("moving to", next, "uses more time", time_cost, "than avail", current.time_left)
        #     pass
        # elif ele[1] == 0:
        #     # no point
        #     pass
        # elif ele[0] in closed:
        #     # already closed
        #     pass
        # else:
        #     next, vent, time_cost = ele
        #     child_path = Path(elephant.path.copy(), [], next)
        #     child_path.path.append(next)
        #     child_path.score = elephant.score + vent
        #     # include time to open valve
        ...
        #     elephant.child_branches.append(child_path)

    # assert len(current.child_branches) < 60, f"{current.child_branches}"
    # assert len(elephant.child_branches) < 60, f"{elephant.child_branches}"

    # print("current branches", len(current_child_branches), "ele branches", len(ele_child_branches))

    # if len(current.child_branches) * len(elephant.child_branches) > 0:
    #     print("child", len(current.child_branches), "ele", len(elephant.child_branches), "=", len(current.child_branches) * len(elephant.child_branches) )

    # prediction_iters += len(current_child_branches) * len(ele_child_branches)
    # for child in current_child_branches:
    #     for ele in ele_child_branches:

    #         if not child.path_set.isdisjoint(ele.path_set):
    #             continue

    #         if abs(len(child.path) - len(ele.path)) > 2:
    #             continue
    #         # check if there is overlap in the paths, these are not candidates to pursue
    #         # further
    #         # cur_set = child.path_set
    #         # ele_set = ele.path_set
    #         # has_intersect = False
    #         # for c in cur_set:
    #         #     if c in ele_set:
    #         #         has_intersect = True
    ...
    #         # was getting a lot of duplicates, so using this as a measure to address this
    #         # considered_set_key = ''.join(child.path) + '-' + ''.join(ele.path)
    #         # if considered_set_key in already_considered:
    #         #     continue

    #         get_path_tree_recurse(child, ele, end_paths, interested_paths, already_considered)
    #         # end_paths.append((child, ele))
    #         #  already_considered.add(considered_set_key)
